...
int32_t open_galaxy_cam_by_SN(galaxy_cam_t* device, char* device_SN)
{
	...
	if (GXSetEnum(device->device_handle, GX_ENUM_TRIGGER_MODE, GX_TRIGGER_MODE_OFF) != GX_STATUS_SUCCESS) //Initially set trigger mode to none
	...
	if (GXGetEnum(device->device_handle, GX_ENUM_PIXEL_COLOR_FILTER, (int64_t*)(&device->pixel_color_format)) != GX_STATUS_SUCCESS)
